...
//  cout << "In loader: " << filename << endl;
	...
	{
		lua_close(pL);
		return("Plugin: " + filename + " is already loaded. Please use the reload command to reload plugin.");
	}
...
string PluginManager::reload(string filename)
{
	...
	string unload_str = unload(filename);
	string load_str = load(filename);

	filename = PLUGIN_DIR + filename;

	// check if either failed
	//return(filename + " was reloaded: " + unload_str + " " + load_str);
	return("UNLOAD: " + unload_str + "\n" + "LOAD: " + load_str);
}
...
string PluginManager::unload(string filename)
{
	...
	return("No such file loaded: " + filename);
